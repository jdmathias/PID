...
Ierror(1)=0;
coe=0.01;
gammaP=1/0.0179*coe;gammaI=1/0.1303*coe;gammaD=1/0.0011*coe;%normalization by the maximum error reached in the reference case

for t=2:npas
    error(t)=Cd(t-1)+Od(t-1);
    if t>2
        Perror(t)=error(t);
...
figure
yyaxis left
plot(2010:5:(2010+(npas-1)*5),C/2.13)%plot(2010:5:(2010+(npas-1)*5),Cref,2010:5:(2010+(npas-1)*5),CCO2,2010:5:(2010+(npas-1)*5),CEco)
yyaxis right
plot(2010:5:(2010+(npas-1)*5),Cd+Od)%plot(2010:5:(2010+(npas-1)*5),Ecoref,2010:5:(2010+(npas-1)*5),EcoCO2,2010:5:(2010+(npas-1)*5),EcoEco)
